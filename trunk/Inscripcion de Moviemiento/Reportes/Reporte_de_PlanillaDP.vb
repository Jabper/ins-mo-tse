...
Public Class Reporte_de_PlanillaDP
    ...
    Private Sub Reporte_de_PlanillaDP_ParametersRequestBeforeShow(ByVal sender As Object, ByVal e As DevExpress.XtraReports.Parameters.ParametersRequestEventArgs) Handles Me.ParametersRequestBeforeShow
        Dim dataset As New DS_REPORTE_DE_DP
        Using Adapter As New DS_REPORTE_DE_DPTableAdapters.DT_PARTIDOS_DPTableAdapter
            Adapter.Fill(dataset.DT_PARTIDOS_DP)
        End Using

        For Each info In e.ParametersInformation
            If info.Parameter.Name = "NombrePartido" Then
                Dim LookUpEdit As New LookUpEdit()
                LookUpEdit.Properties.DataSource = dataset.DT_PARTIDOS_DP
        ...
        Using Adapter1 As New DS_REPORTE_DE_DPTableAdapters.DT_MOVIMIENTO_DPTableAdapter
            Adapter1.Fill(dataset.DT_MOVIMIENTO_DP)
        End Using

        For Each info In e.ParametersInformation
            If info.Parameter.Name = "NombreMovimiento" Then
                Dim LookUpEdit As New LookUpEdit()
                LookUpEdit.Properties.DataSource = dataset.DT_MOVIMIENTO_DP
        ...
        Using Adapter2 As New DS_REPORTE_DE_DPTableAdapters.DT_NIVELE_DPTableAdapter
            Adapter2.Fill(dataset.DT_NIVELE_DP)
        End Using

        For Each info In e.ParametersInformation
            If info.Parameter.Name = "NivelElectivo" Then
                Dim LookUpEdit As New LookUpEdit()
                LookUpEdit.Properties.DataSource = dataset.DT_NIVELE_DP
